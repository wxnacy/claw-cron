...
wave: 4
plan: "03-05"
title: "chat Command — Natural Language Task Management"
depends_on:
  - "03-01"
  - "03-02"
files_modified:
...
# Plan 03-05: chat Command

## Objective

实现 `claw-cron chat` 命令，用自然语言完成任务的增删查、立即执行、启用/禁用。复用 `agent.py` 的 Anthropic tool_use 模式，单轮 intent 识别（不保留历史）。

## Tasks

### Task 1: 创建 cmd/chat.py

<read_first>
- `src/claw_cron/agent.py` — Anthropic tool_use 循环完整实现（_MODEL, tool_use 处理, 消息循环）
- `src/claw_cron/storage.py` — load_tasks, get_task, add_task, delete_task, update_task 函数签名
- `src/claw_cron/executor.py` — execute_task 函数签名
- `src/claw_cron/cmd/add.py` — _add_direct 函数签名（chat add 操作复用）
</read_first>

<action>
创建 `src/claw_cron/cmd/chat.py`：
...
<acceptance_criteria>
- `grep "def chat" src/claw_cron/cmd/chat.py` 有输出
- `grep "_handle_tool" src/claw_cron/cmd/chat.py` 有输出
- `grep "list_tasks\|add_task\|delete_task\|run_task\|enable_task\|disable_task" src/claw_cron/cmd/chat.py | wc -l` 输出 >= 6（6 个工具均定义）
- `grep "update_task" src/claw_cron/cmd/chat.py` 有输出（enable/disable 使用 update_task）
- `grep "_add_direct" src/claw_cron/cmd/chat.py` 有输出（add 操作复用 _add_direct）
- `uv run python -c "from claw_cron.cmd.chat import chat; print('ok')"` 输出 `ok`
</acceptance_criteria>
...
```bash
uv run python -c "
from claw_cron.cmd.chat import chat, _TOOLS, _handle_tool
import click
assert isinstance(chat, click.BaseCommand)
tool_names = {t['name'] for t in _TOOLS}
expected = {'list_tasks', 'add_task', 'delete_task', 'run_task', 'enable_task', 'disable_task'}
assert tool_names == expected, f'Missing tools: {expected - tool_names}'
print('chat command OK')
"
```

## must_haves

- 6 个工具全部实现：list_tasks, add_task, delete_task, run_task, enable_task, disable_task
- 单轮 intent 识别（每条消息独立，不保留历史）
- `exit`/`quit` 可正常退出
- add 操作复用 `_add_direct`，enable/disable 使用 `update_task`
